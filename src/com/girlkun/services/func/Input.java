...
public class Input {
        ...
        try {
            ...
            switch (player.iDMark.getTypeInput()) {
                ...
                case QUY_DOI_COIN:
                    ...
                    int goldTrade = Integer.parseInt(text[0]);// số coin muốn dùng để đổi thỏi vàng
                    if (goldTrade >= 1000000 || goldTrade <= 1000) {
                        Service.gI().sendThongBao(player, "Giới hạn coin nhập lớn hơn 1000 và nhỏ hơn 1 triệu");
                    } else if (goldTrade % 1000 != 0) {
                        Service.gI().sendThongBao(player, "Số coin nhập vào phải chia hết cho 1000");
                    } else if (player.getSession().coinBar >= goldTrade) {
                        PlayerDAO.subcoinBar(player, goldTrade);
                        int goldReceived = (goldTrade * ratioGold) / coinGold;
                        Item goldBar = ItemService.gI().createNewItem((short) 457, goldReceived);
                        InventoryServiceNew.gI().addItemBag(player, goldBar);
                        InventoryServiceNew.gI().sendItemBags(player);
                        Service.gI().sendThongBao(player, "Bạn nhận được " + goldReceived + " "
                                + goldBar.template.name.toUpperCase() + " từ việc quy đổi " + goldTrade + " coin.");
                    } else {
                        Service.gI().sendThongBao(player,
                                "Số tiền còn lại của bạn là " + player.getSession().coinBar + " không đủ để quy "
                                        + " đổi " + (goldTrade * ratioGold) / coinGold + " thỏi vàng. Bạn cần thêm "
                                        + (goldTrade - player.getSession().coinBar));
                    }
                    break;
                case QUY_DOI_HONG_NGOC:
                    int ratioRuby = 50;
                    int rubyTrade = Integer.parseInt(text[0]);// số coin muốn dùng để đổi ruby
                    if (rubyTrade >= 1000000 || rubyTrade <= 1000) {
                        Service.gI().sendThongBao(player, "Giới hạn coin nhập lớn hơn 1000 và nhỏ hơn 1 triệu");
                        ...
                        Service.gI().sendThongBao(player, "Bạn nhận được " +
                                (rubyTrade * ratioRuby) + " ruby.");
                    } else {
                        Service.gI().sendThongBao(player,
                                "Số tiền còn lại của bạn là " + player.getSession().coinBar + " không đủ để quy "
                                        + " đổi " + rubyTrade * ratioRuby + " ruby. Bạn cần thêm "
    ...
    public void createFormQDTV(Player pl) {
        ...
        createForm(pl, QUY_DOI_COIN, "Quy đổi thỏi vàng, giới hạn đổi không quá 500 "
                ...
                + "\n500.000 Vnd = 1.500 Thỏi vàng", new SubInput("Nhập số lượng coin muốn đổi", NUMERIC));
    }

    public void createFormQDHN(Player pl) {

        createForm(pl, QUY_DOI_HONG_NGOC, "Quy đổi hồng ngọc"
                + "\n10.000 Vnd = 500.000 Hồng ngọc "
                + "\n20.000 Vnd = 1.000.000 Hồng ngọc "
                + "\n50.000 Vnd = 2.500.000 Hồng ngọc "
                + "\n100.000 Vnd = 5.000.000 Hồng ngọc "
                + "\n200.000 Vnd = 10.000.000 Hồng ngọc "
                + "\n500.000 Vnd = 25.000.000 Hồng ngọc", new SubInput("Nhập số lượng coin muốn đổi", NUMERIC));
    }
